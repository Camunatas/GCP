...
class Class_scenred():
    ...
    def import_data(self, file_dir, **kwargs):      # Imports data from the file
        if isinstance(file_dir, str) == False:      # Raises exception if data is not in dictionary format
            raise Exception("No such file in the directory.")
        
        f = h5py.File(file_dir, "r")                # Assigns generate a file object from file_dir and stores it in variable f
        self.name_list = list(f.keys())             # Generates instance variable with list of keys on f
        
        d = dict()
        d['param1'] = f['param1'][()]
        # Generates empty dictionary d
        # for name in self.name_list:                 # Stores keys on dictionary d
        #     d[name] = f[name][()]
        #     #[()] is to ndarray. see https://stackoverflow.com/questions/10274476/how-to-export-hdf5-file-to-numpy-using-h5py
            
        self.data = d                               # Generates instance variable data with the dictionary d
    
    def prepare_data(self, **kwargs):               # Generates instance variables from the instance variable self.data
        if isinstance(self.data, dict) == False:  #input data should be a dict.
            raise Exception("The input data is not a dictionary.")
        
        self.dim_DAT = len(self.data)               # Generates instance variable dim_DAT with data dictionary dimensions (2)
        
        self.data_agg = np.hstack(self.data.values())   # Concatenates values from different keys but placed at the same position
        self.dim_M, self.dim_N = self.data_agg.shape[0], int(self.data_agg.shape[1] /self.dim_DAT)  # Generates instance variables dim_M and dim_N with data_agg first and second dimension
    ...
    def scenario_reduction(self, **kwargs):    
        ...
        dim_M, dim_N = self.dim_M, self.dim_N #dim_M= scenarios, dim_N=hours
        ...
        return data_out, m_epo_res, m_epo_link, m_epo_prob
    ...
    #draw the scenario reduction figure
